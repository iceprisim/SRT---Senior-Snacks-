...
@Config
@Autonomous(name = "ORedAutoBB", group = "Autonomous")
public class ORedAutoBB extends LinearOpMode {
    ...
    public class Shooter {
        ...
        public class Shoot1 implements Action{
            ...
            public boolean run(@NonNull TelemetryPacket packet){
                shooterLeft.setPower(0.69);
                shooterRight.setPower(0.69);
                return false;
            }
        }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        // start pose
        Pose2d beginPose = new Pose2d(new Vector2d(0, 0), 3.14/2);//0

        // build drive
        MecanumDrive drive = new MecanumDrive(hardwareMap, beginPose);

        // build trajectories
        TrajectoryActionBuilder Sideways = drive.actionBuilder(beginPose)
                //.lineToY(1)
                .splineToSplineHeading(new Pose2d(0, -1, 0.6), Math.PI / 2);
                //.lineToXSplineHeading(-30, -0.45);

        TrajectoryActionBuilder sleep = drive.actionBuilder(
                        new Pose2d(new Vector2d(3.5, 10), 0.45))
                .waitSeconds(10);
