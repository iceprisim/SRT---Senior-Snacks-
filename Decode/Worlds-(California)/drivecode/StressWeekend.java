...
public class StressWeekend extends LinearOpMode {
    ...
    private boolean bumperIsHolding = false;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad2.dpad_up) //Mid Range
                {ShootAngle.setPosition(0.181);
                ShooterPower = 0.73;}
                
            if (gamepad2.dpad_left) // XX
                {ShootAngle.setPosition(0.17);
                ShooterPower = 0.59;}
            
            if (gamepad2.dpad_down) //XX
                {ShootAngle.setPosition(0.155);
                ShooterPower = 1;}
           ...
           //=========================================================
            ...
            if (gamepad2.leftBumperWasPressed()) {
                bumperPressTime = System.currentTimeMillis();//gets the time the button is held
                bumperIsHolding = false;
            }

            if (gamepad2.left_bumper && 
                (System.currentTimeMillis() - bumperPressTime) > 200) {
                bumperIsHolding = true; //if ur holding it
            }
            //------------if ur not holding/toggle aka STOPING------------
            if (gamepad2.leftBumperWasReleased()) {
                if (bumperIsHolding) {
                    conveyorTogglePower = 0;//stops it
                } 
                else {
                    if (conveyorTogglePower == 0) {
                        conveyorTogglePower = 0.9;
                    }
                    else {
            ...
            //------------converyour output either being slow or regular------------
            if (bumperIsHolding) {
                conveyorOutput = 0.4;//holding
            } 
            else {
                conveyorOutput = conveyorTogglePower;//toggles
            }
            //------------trigger reverse------------
            if (gamepad2.left_trigger > 0.05) {
                conveyorOutput = -0.5 * gamepad2.left_trigger;
            }
